...
void SysCountryInformation::DemographicsTab() {
    ...
    for (auto& seg_entity : settlement.population) {
        ...
        if (GetUniverse().any_of<cqspc::Employee>(seg_entity)) {
...
template <typename T>
void SysCountryInformation::IndustryTabGenericChild(const std::string& tabname, const std::string& industryname,
                                                    const ImVec2& size) {
