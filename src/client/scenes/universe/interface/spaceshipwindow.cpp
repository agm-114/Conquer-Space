...
<<<<<<< HEAD
                 void SpaceshipWindow::Init() {} == == == = void SpaceshipWindow::DoUI(int delta_time) {
...
<<<<<<< HEAD
            ...
            if (ImGui::CollapsingHeader("Basic Orbital Maneuvers")) {
                ...
                double apoapsis_velocity = OrbitVelocity(common::components::types::apoapsis, orbit.eccentricity,
...
== == == = if (ImGui::Button("Circularize at apoapsis")) {
    // Add random delta v
    common::systems::commands::PushManeuvers(GetUniverse(), body, {common::systems::CircularizeAtApoapsis(orbit)});
}
if (ImGui::IsItemHovered()) {
...
if (ImGui::Button("Leave SOI")) {
    auto maneuver =
        common::systems::TransferFromBody(GetUniverse(), orbit, GetUniverse().get<Kinematics>(body), transfer_radius);
    PushManeuver(GetUniverse(), body, maneuver);
                    ...
                    if (ImGui::CollapsingHeader("Rendezvous")) {
                        ...
                        static entt::entity selected =
                            o_system.children.empty() ? entt::null : o_system.children.front();
                        auto& target = GetUniverse().get<Orbit>(selected);

                        // Get distance from target
                        glm::dvec3 target_distance = GetUniverse().get<Kinematics>(selected).position -
...
>>>>>>> pr-288
                                auto& coords = GetUniverse().get<Kinematics>(body);
                                ImGui::TextFmt("Position {} {} {}", coords.position.x, coords.position.y,
                                               coords.position.z);
                                ImGui::TextFmt("Velocity {} {} {}", coords.velocity.x, coords.velocity.y,
                                               coords.velocity.z);
                                == == == = namespace cqsp::client::systems {
                                    ...
                                    void SpaceshipWindow::DoUI(int delta_time) {
                                        ...
                                        // Display orbital elements
                                        auto& orbit = GetUniverse().get<Orbit>(body);
                                        if (ImGui::CollapsingHeader("Orbital Elements",
                                                                    ImGuiTreeNodeFlags_DefaultOpen)) {
                                            ImGui::TextFmt("Semi-major axis: {} km", orbit.semi_major_axis);
                                            ImGui::TextFmt("Eccentricity: {}", orbit.eccentricity);
                                            // Degree symbol is broken :(
                                            ImGui::TextFmt("Inclination: {}\u00b0", toDegree(orbit.inclination));
                                            ImGui::TextFmt("Longitude of the ascending node: {}\u00b0",
                                                           toDegree(orbit.LAN));
                                            ImGui::TextFmt("Argument of periapsis: {}\u00b0", toDegree(orbit.w));
                                            ImGui::TextFmt("Mean anomaly at Epoch: {}\u00b0", toDegree(orbit.M0));
                                            ImGui::TextFmt("True anomaly: {}", orbit.v);
                                            ...
                                            ImGui::TextFmt("GM: {} km^3 * s^-2", orbit.GM);
                                            ImGui::TextFmt("Orbital period: {} s", orbit.T());
                                            ImGui::TextFmt("Orbiting: {}",
                                                           GetName(GetUniverse(), orbit.reference_body).c_str());
                                            if (GetUniverse().valid(orbit.reference_body) &&
                                                GetUniverse().any_of<bodies::Body>(orbit.reference_body)) {
                                                double r = orbit.GetOrbitingRadius();
                                                double p = GetUniverse().get<bodies::Body>(orbit.reference_body).radius;
                                                ...
                                                ImGui::TextFmt("Periapsis: {:<10} km ({:.1f} s)",
                                                               orbit.GetPeriapsis() - p, orbit.TimeToTrueAnomaly(0));
                                                ImGui::TextFmt("Apoapsis: {:<10} km ({:.1f} s)",
                                                               orbit.GetApoapsis() - p,
                                                               orbit.TimeToTrueAnomaly(common::components::types::PI));
                                            }
                                        }
                                        ...
                                        if (ImGui::CollapsingHeader("Basic Orbital Maneuvers")) {
                                            ...
                                            double apoapsis_velocity =
                                                OrbitVelocity(common::components::types::PI, orbit.eccentricity,
                                                              orbit.semi_major_axis, orbit.GM);
                                            double periapsis_velocity =
                                            ...
                                            if (ImGui::Button("Fix True anomaly")) {
                                                // Emplace
                                                GetUniverse().emplace_or_replace<types::SetTrueAnomaly>(body,
                                                                                                        true_anomaly);
                                            }
                                            ...
                                            if (ImGui::Button("Circularize at apoapsis")) {
                                                // Add random delta v
                                                common::systems::commands::PushManeuvers(
                                                    GetUniverse(), body,
                                                    {common::systems::CircularizeAtApoapsis(orbit)});
                                            }
                                            if (ImGui::IsItemHovered()) {
                                                double circular_velocity =
                                                    GetCircularOrbitingVelocity(orbit.GM, orbit.GetApoapsis());
                                                // So for apoapsis, we need this amount of delta v at prograde
                                                // Get the vector of the direction and then compute?
                                                // Then transform by the orbital math
                                                ImGui::SetTooltip("Delta-v: %f km/s",
                                                                  circular_velocity - apoapsis_velocity);
                                            }

                                            if (ImGui::Button("Circularize at perapsis")) {
                                                PushManeuvers(GetUniverse(), body,
                                                              {common::systems::CircularizeAtPeriapsis(orbit)});
                                            }

                                            if (ImGui::IsItemHovered()) {
                                                double circular_velocity =
                                                    GetCircularOrbitingVelocity(orbit.GM, orbit.GetPeriapsis());
                                                // So for apoapsis, we need this amount of delta v at prograde
                                                // Get the vector of the direction and then compute?
                                                // Then transform by the orbital math
                                                ImGui::SetTooltip("Delta-v: %f km/s",
                                                                  circular_velocity - periapsis_velocity);
                                            }

                                            // Change apogee
                                            ImGui::InputDouble("Set Apoapsis Altitude", &new_perigee, min_altitude,
                                                               max_altitude);
                                            if (ImGui::Button("Set Apoapsis")) {
                                                // Get velocity at the new apogee
                                                // Get the velocity
                                            ...
                                            if (ImGui::IsItemHovered()) {
                                                double new_sma = (orbit.GetApoapsis() + new_perigee) / 2;
                                                // Get velocity at the new apogee
                                                double new_v = common::components::types::OrbitVelocityAtR(
                                                    orbit.GM, new_sma, orbit.GetApoapsis());
                                                ImGui::SetTooltip("Delta-v: %f km/s", new_v - periapsis_velocity);
                                            }

                                            // Change apogee
                                            ImGui::InputDouble("Set Periapsis Altitude", &new_apogee, min_altitude,
                                                               max_altitude);
                                            if (ImGui::Button("Set Periapsis")) {
                                                // Get velocity at the new apogee
                                                // Get the velocity
                                            ...
                                            if (ImGui::IsItemHovered()) {
                                                ...
                                                // Get velocity at the new apogee
                                                double new_v = common::components::types::OrbitVelocityAtR(
                                                    orbit.GM, new_sma, orbit.GetPeriapsis());
                                                ImGui::SetTooltip("Delta-v: %f km/s", new_v - periapsis_velocity);
                                            }

                                            ImGui::InputDouble("Set Orbit Altitude", &new_hohmann, min_altitude,
                                                               max_altitude);
                                            if (ImGui::Button("Set New Orbit")) {
                                                // Get velocity at the new apogee
                                                // Get the velocity
                                        ...
                                        if (ImGui::CollapsingHeader("Rendezvous")) {
                                            auto& o_system =
                                                GetUniverse().get<bodies::OrbitalSystem>(orbit.reference_body);
                                            static entt::entity selected =
                                                o_system.children.empty() ? entt::null : o_system.children.front();
                                            auto& target = GetUniverse().get<Orbit>(selected);
                                            ...
                                            glm::vec3 target_distance =
                                                GetUniverse().get<Kinematics>(selected).position -
                                                GetUniverse().get<Kinematics>(body).position;
                                            ImGui::TextFmt("Distance to target: {}", glm::length(target_distance));
                                            if (ImGui::Button("Rendez-vous!")) {
                                                // Rdv with target
                                                auto pair = cqsp::common::systems::CoplanarIntercept(
                                                    orbit, target, GetUniverse().date.ToSecond());
                                                common::systems::commands::PushManeuvers(GetUniverse(), body, pair);
                                            }
                                            if (ImGui::Button("Maneuver to point")) {
                                            ...
                                            ImGui::TextFmt("Phase angle: {}",
                                                           cqsp::common::components::types::CalculatePhaseAngle(
                                                               orbit, target, GetUniverse().date.ToSecond()));
                                            ImGui::TextFmt(
                                                "Transfer angle: {}",
                                                cqsp::common::components::types::CalculateTransferAngle(orbit, target));
                                            double ttma = orbit.TimeToTrueAnomaly(
                                                common::components::types::AscendingTrueAnomaly(orbit, target));
                                            if (ImGui::Button("Lambert transfer")) {
                                                // Compute lambert problem
                                                // Get future position of the entity
                                            ...
                                            ImGui::SliderFloat("Lambert Time of Flight (seconds)", &lambert_tof, 0,
                                                               1e6);
                                            ImGui::TextFmt("Time to ascending node: {}", ttma);
                                            if (ImGui::BeginChild("Rendezvous Target")) {
                                                for (auto& entity : o_system.children) {
                                                    if (ImGui::Selectable(GetName(GetUniverse(), entity).c_str(),
                                                                          selected == entity)) {
                                                        selected = entity;
                                                    }
                                                }
                                        ...
                                        if (ImGui::CollapsingHeader("Moon Transfers")) {
                                            ...
                                            if (ImGui::Button("Transfer to Moon")) {
                                                common::systems::commands::TransferToMoon(GetUniverse(), body,
                                                                                          selected);
                                            }
...
<<<<<<< HEAD
                                                            == == == = if (GetUniverse().any_of<CommandQueue>(body)) {
                                                                ...
                                                                for (auto& manuver : queue) {
                                                                    ImGui::TextFmt(
                                                                        "Maneuver in {}",
                                                                        manuver.time - GetUniverse().date.ToSecond());
>>>>>>> pr_254
                                                                }
                                                                == == == =
                                                            }
                                                    ...
                                                    if (ImGui::CollapsingHeader("Basic Orbital Maneuvers")) {
                                                        ...
                                                        double apoapsis_velocity = OrbitVelocity(
                                                            common::components::types::PI, orbit.eccentricity,
                                                            orbit.semi_major_axis, orbit.GM);
                                                        double periapsis_velocity = OrbitVelocity(
                                                            0, orbit.eccentricity, orbit.semi_major_axis, orbit.GM);
                                                        double max_altitude = 10000000;
                                                        double min_altitude = 0;

                                                        if (ImGui::Button("Fix True anomaly")) {
                                                            // Emplace
                                                            GetUniverse().emplace_or_replace<types::SetTrueAnomaly>(
                                                                body, true_anomaly);
                                                        }
                                                        if (GetUniverse().any_of<types::SetTrueAnomaly>(body)) {
                                                            // Now set the true anomaly consistently or something
                                                        ...
                                                        if (ImGui::IsItemHovered()) {
                                                            double circular_velocity = GetCircularOrbitingVelocity(
                                                                orbit.GM, orbit.GetApoapsis());
                                                            // So for apoapsis, we need this amount of delta v at prograde
                                                            // Get the vector of the direction and then compute?
                                                            // Then transform by the orbital math
                                                            ImGui::SetTooltip("Delta-v: %f km/s",
                                                        ...
                                                        // Change apogee
                                                        ImGui::InputDouble("Set Apoapsis Altitude", &new_perigee,
                                                                           min_altitude, max_altitude);
                                                        if (ImGui::Button("Set Apoapsis")) {
                                                            // Get velocity at the new apogee
                                                            // Get the velocity
                                                            auto maneuver =
                                                                common::systems::SetApoapsis(orbit, new_perigee);
                                                            PushManeuvers(GetUniverse(), body, {maneuver});
                                                        }
                                                        ...
                                                        if (ImGui::IsItemHovered()) {
                                                            ...
                                                            double new_v = common::components::types::OrbitVelocityAtR(
                                                                orbit.GM, new_sma, orbit.GetApoapsis());
                                                            ImGui::SetTooltip("Delta-v: %f km/s",
                                                                              new_v - periapsis_velocity);
                                                        }

                                                        // Change apogee
                                                        ImGui::InputDouble("Set Periapsis Altitude", &new_apogee,
                                                                           min_altitude, max_altitude);
                                                        if (ImGui::Button("Set Periapsis")) {
                                                            // Get velocity at the new apogee
                                                            // Get the velocity
                                                            auto maneuver =
                                                                common::systems::SetPeriapsis(orbit, new_apogee);
                                                            PushManeuvers(GetUniverse(), body, {maneuver});
                                                        }
                                                        ...
                                                        ImGui::InputDouble("Set Orbit Altitude", &new_hohmann,
                                                                           min_altitude, max_altitude);
                                                        if (ImGui::Button("Set New Orbit")) {
                                                            // Get velocity at the new apogee
                                                            // Get the velocity
                                                            auto hohmann =
                                                                common::systems::HohmannTransfer(orbit, new_hohmann);
                                                            if (hohmann.has_value()) {
                                                                PushManeuvers(GetUniverse(), body, *hohmann);
                                                            } else {
                                                                SPDLOG_INFO("Orbit is not circular!");
                                                    ...
                                                    // Display controls of the spaceship
                                                    if (ImGui::CollapsingHeader("Rendezvous")) {
                                                        auto& o_system = GetUniverse().get<bodies::OrbitalSystem>(
                                                            orbit.reference_body);
                                                        static entt::entity selected = o_system.children.empty()
                                                                                           ? entt::null
                                                                                           : o_system.children.front();
                                                        auto& target = GetUniverse().get<Orbit>(selected);
                                                        ...
                                                        glm::vec3 target_distance =
                                                            GetUniverse().get<Kinematics>(selected).position -
                                                            GetUniverse().get<Kinematics>(body).position;
                                                        ImGui::TextFmt("Distance to target: {}",
                                                                       glm::length(target_distance));
                                                        if (ImGui::Button("Rendez-vous!")) {
                                                            // Rdv with target
                                                            auto pair = cqsp::common::systems::CoplanarIntercept(
                                                                orbit, target, GetUniverse().date.ToSecond());
                                                            common::systems::commands::PushManeuvers(GetUniverse(),
                                                                                                     body, pair);
                                                        }
                                                        if (ImGui::Button("Maneuver to point")) {
                                                            auto pair = cqsp::common::systems::CoplanarIntercept(
                                                                orbit, target, GetUniverse().date.ToSecond());
                                                            common::systems::commands::PushManeuvers(
                                                                GetUniverse(), body, {pair.first});
                                                        }
                                                        if (ImGui::Button("Match Planes")) {
                                                            auto maneuver =
                                                                cqsp::common::systems::MatchPlanes(orbit, target);
                                                            common::systems::commands::PushManeuvers(GetUniverse(),
                                                                                                     body, {maneuver});
                                                        }
                                                        ImGui::TextFmt(
                                                        ...
                                                        ImGui::TextFmt(
                                                            "Transfer angle: {}",
                                                            cqsp::common::components::types::CalculateTransferAngle(
                                                                orbit, target));
                                                        double ttma = orbit.TimeToTrueAnomaly(
                                                            common::components::types::AscendingTrueAnomaly(orbit,
                                                                                                            target));
                                                        if (ImGui::Button("Lambert transfer")) {
                                                            // Compute lambert problem
                                                            // Get future position of the entity
                                                        ...
                                                        ImGui::SliderFloat("Lambert Time of Flight (seconds)",
                                                                           &lambert_tof, 0, 1e6);
                                                        ImGui::TextFmt("Time to ascending node: {}", ttma);
                                                        if (ImGui::BeginChild("Rendezvous Target")) {
                                                            for (auto& entity : o_system.children) {
                                                                if (ImGui::Selectable(
                                                                        GetName(GetUniverse(), entity).c_str(),
                                                                        selected == entity)) {
                                                                    selected = entity;
>>>>>>> pr-288
                                                                }
...
<<<<<<< HEAD
                                                                double apoapsis_velocity = OrbitVelocity(
                                                                    ...
                                                                    orbit.semi_major_axis, orbit.GM);
                                                                double periapsis_velocity =
                                                                    OrbitVelocity(0, orbit.eccentricity,
                                                                                  orbit.semi_major_axis, orbit.GM);
                                                                double max_altitude = 10000000;
                                                                double min_altitude = 0;
                                                                ...
                                                                if (ImGui::Button("Fix True anomaly")) {
                                                                    // Emplace
                                                                    GetUniverse()
                                                                        .emplace_or_replace<types::SetTrueAnomaly>(
                                                                            body, true_anomaly);
                                                                    == == ==
                                                                        = if (ImGui::BeginChild("Landing Target")) {
                                                                        for (auto& entity : o_system.children) {
                                                                            if (!GetUniverse()
                                                                                     .all_of<bodies::Planet, Orbit>(
                                                                                         entity)) {
                                                                                continue;
>>>>>>> pr-288
                                                                            }
                                                                            if (GetUniverse()
                                                                                    .any_of<types::SetTrueAnomaly>(
                                                                                        body)) {
                                                                                // Now set the true anomaly consistently or something
                                                                            }
                                                                            if (ImGui::Button(
...
<<<<<<< HEAD
                                                                            ...
                                                                            if (ImGui::IsItemHovered()) {
                                                                                ...
                                                                                GetUniverse()
                                                                                    .get_or_emplace<CommandQueue>(body)
                                                                                    .commands.push_back(maneuver);
                                                                            }
                                                                            if (ImGui::IsItemHovered()) {
                                                                                double circular_velocity =
                                                                                    types::GetCircularOrbitingVelocity(
                                                                                        orbit.GM, orbit.GetApoapsis());
                                                                                // So for apoapsis, we need this amount of delta v at prograde
                                                                                // Get the vector of the direction and then compute?
                                                                                // Then transform by the orbital math
                                                                                double orbit_velocity = OrbitVelocity(
                                                                                    types::PI, orbit.eccentricity,
                                                                                    orbit.semi_major_axis, orbit.GM);
                                                                                ImGui::SetTooltip(
                                                                                    "Delta-v: %f km/s",
                                                                                    circular_velocity - orbit_velocity);
                                                                            }
...
>>>>>>> pr_254
                                                                            ...
                                                                            // Change apogee
                                                                            ImGui::InputDouble(
                                                                                "Set Apoapsis Altitude", &new_perigee,
                                                                                min_altitude, max_altitude);
                                                                            if (ImGui::Button("Set Apoapsis")) {
                                                                                // Get velocity at the new apogee
                                                                                // Get the velocity
<<<<<<< HEAD
                                                                                auto maneuver =
                                                                                    common::systems::SetApoapsis(
                                                                                        orbit, new_perigee);
                                                                                PushManeuvers(GetUniverse(), body,
                                                                                              {maneuver});
                                                                                == == == = Maneuver maneuver;
                                                                                auto m = common::systems::SetApoapsis(
                                                                                    orbit, new_perigee);
...
>>>>>>> pr_254
                                                                            ...
                                                                            if (ImGui::IsItemHovered()) {
                                                                                double new_sma = (orbit.GetApoapsis() +
                                                                                                  new_perigee) /
                                                                                                 2;
                                                                                // Get velocity at the new apogee
<<<<<<< HEAD
                                                                                double new_v = common::components::
                                                                                    types::OrbitVelocityAtR(
                                                                                        orbit.GM, new_sma,
                                                                                        orbit.GetApoapsis());
                                                                                ImGui::SetTooltip(
                                                                                    "Delta-v: %f km/s",
                                                                                ...
                                                                                ImGui::SetTooltip(
                                                                                    "Delta-v: %f km/s",
                                                                                    new_v - orbit_velocity);
>>>>>>> pr_254
                                                                            }

                                                                            // Change apogee
                                                                            ImGui::InputDouble(
                                                                                "Set Periapsis Altitude", &new_apogee,
                                                                                min_altitude, max_altitude);
                                                                            if (ImGui::Button("Set Periapsis")) {
                                                                                // Get velocity at the new apogee
                                                                                // Get the velocity
<<<<<<< HEAD
                                                                                auto maneuver =
                                                                                    common::systems::SetPeriapsis(
                                                                                        orbit, new_apogee);
                                                                                PushManeuvers(GetUniverse(), body,
                                                                                              {maneuver});
                                                                                == == == = Maneuver maneuver;
                                                                                auto m = common::systems::SetPeriapsis(
                                                                                    orbit, new_apogee);
                                                                                maneuver.delta_v = m.first;
                                                                                maneuver.time =
                                                                                    GetUniverse().date.ToSecond() +
...
>>>>>>> pr_254
                                                                            ...
                                                                            if (ImGui::IsItemHovered()) {
                                                                                double new_sma = (orbit.GetPeriapsis() +
                                                                                                  new_apogee) /
                                                                                                 2;
                                                                                // Get velocity at the new apogee
<<<<<<< HEAD
...
>>>>>>> pr_254
                                                                            ...
                                                                            ImGui::InputDouble(
                                                                                "Set Orbit Altitude", &new_hohmann,
                                                                                min_altitude, max_altitude);
                                                                            if (ImGui::Button("Set New Orbit")) {
                                                                                // Get velocity at the new apogee
                                                                                // Get the velocity
                                                                                auto hohmann =
                                                                                    common::systems::HohmannTransfer(
                                                                                        orbit, new_hohmann);
                                                                                if (hohmann.has_value()) {
<<<<<<< HEAD
                                                                                    PushManeuvers(GetUniverse(), body,
...
<<<<<<< HEAD
                                                                            auto& o_system =
                                                                                GetUniverse()
                                                                                    .get<bodies::OrbitalSystem>(
                                                                                        orbit.reference_body);
                                                                            static entt::entity selected =
                                                                                o_system.children.empty()
                                                                                    ? entt::null
                                                                                    : o_system.children.front();
                                                                            auto& target =
                                                                                GetUniverse().get<Orbit>(selected);

                                                                            // Get distance from target
                                                                            glm::dvec3 target_distance =
                                                                                GetUniverse()
                                                                                    .get<Kinematics>(selected)
                                                                                    .position -
                                                                                GetUniverse()
                                                                                    .get<Kinematics>(body)
                                                                                    .position;
...
<<<<<<< HEAD
                                                                            ...
                                                                            if (ImGui::Button("Maneuver to point")) {
                                                                                ...
                                                                                auto& queue =
                                                                                    GetUniverse()
                                                                                        .get_or_emplace<CommandQueue>(
                                                                                            body);
                                                                                common::components::Maneuver man_1(
                                                                                    pair.first);
                                                                                man_1.time +=
                                                                                    GetUniverse().date.ToSecond();
                                                                                queue.commands.push_back(man_1);
...
>>>>>>> pr_254
                                                                                auto maneuver =
                                                                                    cqsp::common::systems::MatchPlanes(
                                                                                        orbit, target);
                                                                                common::systems::commands::PushManeuver(
                                                                                    GetUniverse(), body, maneuver);
                                                                            }
<<<<<<< HEAD
                                                                            ImGui::TextFmt(
                                                                                "Phase angle: {}",
                                                                                cqsp::common::components::types::
                                                                                    CalculatePhaseAngle(
                                                                                        orbit, target,
                                                                                        GetUniverse().date.ToSecond()));
                                                                            ImGui::TextFmt(
                                                                                "Transfer angle: {}",
                                                                                cqsp::common::components::types::
                                                                                    CalculateTransferAngle(orbit,
                                                                                                           target));
                                                                            double ttma = orbit.TimeToTrueAnomaly(
                                                                                common::components::types::
                                                                                    AscendingTrueAnomaly(orbit,
                                                                                                         target));
                                                                            if (ImGui::Button("Lambert transfer")) {
                                                                                // Compute lambert problem
                                                                                // Get future position of the entity
                                                                            ...
                                                                            == == ==
                                                                                = ImGui::TextFmt(
                                                                                    "Phase angle: {}",
                                                                                    types::CalculatePhaseAngle(
                                                                                        orbit, target,
                                                                                        GetUniverse().date.ToSecond()));
                                                                            ImGui::TextFmt(
                                                                                "Transfer angle: {}",
                                                                                types::CalculateTransferAngle(orbit,
                                                                                                              target));
                                                                            double ttma = orbit.TimeToMeanAnomaly(
                                                                                types::AscendingTrueAnomaly(orbit,
                                                                                                            target));
>>>>>>> pr_254
                                                                            ImGui::TextFmt("Time to ascending node: {}",
                                                                                           ttma);
                                                                            if (ImGui::BeginChild(
                                                                                    "Rendezvous Target")) {
                                                                                for (auto& entity : o_system.children) {
                                                                        ...
                                                                        if (ImGui::CollapsingHeader("Moon Transfers")) {
                                                                            auto& o_system =
                                                                                GetUniverse()
                                                                                    .get<bodies::OrbitalSystem>(
                                                                                        orbit.reference_body);
                                                                            double angle =
                                                                                common::components::types::AngleWith(
                                                                                    orbit,
                                                                                    GetUniverse().get<types::Orbit>(
                                                                                        orbit.reference_body));
                                                                            ImGui::TextFmt("Angle: {}",
                                                                                           angle * 180 / types::PI);
                                                                            static entt::entity selected = entt::null;
                                                                            ...
                                                                            if (ImGui::Button("Transfer to Moon")) {
                                                                                common::systems::commands::
                                                                                    TransferToMoon(GetUniverse(), body,
                                                                                                   selected);
                                                                            }
