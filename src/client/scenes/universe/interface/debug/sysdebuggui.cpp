...
>>>>>>> pr-286
            ...
            void SysDebugMenu::CqspMetricsWindow() {
                ...
                ImPlot::SetNextAxesLimits(GetApp().GetTime() - fps_history_len, GetApp().GetTime(), 0, 300,
                                          ImGuiCond_Always);
                if (ImPlot::BeginPlot("FPS", ImVec2(-1, 0), ImPlotFlags_NoChild)) {
                    ...
                    ImPlot::PlotLine("FPS", &fps_history[0].x, &fps_history[0].y, fps_history.size(), 0,
                                     sizeof(float) * 2);
                    ImPlot::EndPlot();
                }

                ImPlot::SetNextAxisLimits(ImAxis_X1, GetApp().GetTime() - fps_history_len, GetApp().GetTime(),
                                          ImGuiCond_Always);
...
<<<<<<< HEAD
                        entt::entity ent = universe.view<MouseOverEntity>().front();
                        == == == = entity ent = universe.view<MouseOverEntity>().front();
>>>>>>> pr_254
                        if (ent == entt::null) {
                            input.push_back(fmt::format("Mouse is over null"));
                        } else {
                    ...
                    auto entitycount = [](sysdebuggui_parameters) {
                        input.push_back(
                            fmt::format("Total entities: {}, Alive: {}", universe.size(), universe.alive()));
                    };
...
<<<<<<< HEAD
                            if (std::all_of(args.begin(), args.end(), ::isdigit)) {
                                entt::entity entity = static_cast<entt::entity>(atoi(args.data()));
                                == == == = if (std::all_of(args.begin(), args.end(), isdigit)) {
                                    entity entity = static_cast<entt::entity>(atoi(args.data()));
>>>>>>> pr_254
                                    std::string name = "N/A";
                                    if (universe.all_of<components::Name>(entity)) {
                                        name = universe.get<components::Name>(entity);
                    ...
                    void SysDebugMenu::CqspMetricsWindow() {
                        ...
                        ImPlot::SetNextAxisLimits(ImAxis_X1, GetApp().GetTime() - fps_history_len, GetApp().GetTime(),
                                                  ImGuiCond_Always);
                        if (ImPlot::BeginPlot("Profiler", ImVec2(-1, 0),
...
<<<<<<< HEAD
                    void SysDebugMenu::ShowWindows(double delta_time) {
                        == == == = void SysDebugMenu::ShowWindows() {
>>>>>>> pr_254
                                     if (to_show_imgui_about) {ImGui::ShowAboutWindow(&to_show_imgui_about);
                }
                if (to_show_metrics_window) {
                    ImGui::ShowMetricsWindow(&to_show_metrics_window);
                }
                ...
                if (ImGui::InputText("DebugInput", &command,
                    ...
                    if (!command_request.empty()) {
                        ...
                        for (auto it = commands.begin(); it != commands.end(); it++) {
                            ...
                            it->second.second(
                                GetApp(), GetUniverse(), GetScriptInterface(),
            ...
            void SysDebugMenu::DoUI(int delta_time) {
                ...
                ImGui::SetNextWindowSize(
                    ImVec2(ImGui::GetIO().DisplaySize.x * .5f, ImGui::GetIO().DisplaySize.y * 0.75f), ImGuiCond_Always);
                ImGui::SetNextWindowBgAlpha(0.6f);
                ImGui::Begin("Debug Gui", &to_show_window,
                             ImGuiWindowFlags_MenuBar | window_flags | ImGuiWindowFlags_NoDecoration);
                CreateMenuBar();
                DrawConsole();
