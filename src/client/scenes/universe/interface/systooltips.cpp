...
#include "engine/gui.h"
    ...
    <<<<<<< HEAD namespace cqsp::client::systems::gui { namespace components = common::components; namespace types = components::types;
            ...
            == == == = void EntityTooltipContent(const Universe& universe, entt::entity entity) {
                ...
                if (universe.all_of<types::Orbit>(entity)) {
                    ...
                    ImGui::TextFmt("Orbital Period: {} y {} d {} h {} m {} s", (int)(orbit.T() / (60 * 60 * 24 * 365)),
                                   ...
                                   (int)std::fmod(orbit.T() / (60 * 60), 60), (int)std::fmod(orbit.T() / 60, 60),
                                   std::fmod(orbit.T(), 60));
                }
...
>>>>>>> pr_254
                                ...
                                if (universe.all_of<components::infrastructure::PowerConsumption>(entity)) {
                                    ImGui::Separator();
                                    auto& consumption =
                                        universe.get<components::infrastructure::PowerConsumption>(entity);
                                    ImGui::TextFmt("Power: {}", consumption.current);
                                    ImGui::TextFmt("Max Power: {}", consumption.max);
                                    ImGui::TextFmt("Min Power: {}", consumption.min);
                                }
...
<<<<<<< HEAD
                                    if (universe.all_of<Orbit, types::Kinematics>(entity)) {
                                        ...
                                        if (universe.valid(ref) && universe.any_of<Body>(ref)) {
                                            const double radius = universe.get<Body>(ref).radius;
                                            == == == = if (universe.all_of<types::Orbit, types::Kinematics>(entity)) {
                                                auto ref = universe.get<types::Orbit>(entity).reference_body;
                                                if (universe.valid(ref) && universe.any_of<bodies::Body>(ref)) {
                                                    const double radius = universe.get<bodies::Body>(ref).radius;
>>>>>>> pr_254
                                                    double distance =
