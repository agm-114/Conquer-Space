...
void ModelScene::Ui(float deltaTime) {} == == == = namespace cqsp::client::scene {
    void ModelScene::Init() {
        // Load the model and shaders
        model = GetApp().GetAssetManager().GetAsset<asset::Model>("core:iss");
        // Load shader
...
>>>>>>> pr-286
        ...
        void ModelScene::Render(float deltaTime) {
            ...
            model_transform = glm::scale(
                model_transform, glm::vec3(4.0f, 4.0f, 4.0f));  // it's a bit too big for our scene, so scale it down
