...
void conquerspace::scene::UniverseScene::Init() {
    ...
    simulation = std::make_shared<cqspco::systems::simulation::Simulation>(GetApp().GetUniverse(),
                                                                           GetApp().GetScriptInterface());

    system_renderer = new conquerspace::client::systems::SysStarSystemRenderer(GetApp().GetUniverse(), GetApp());
    system_renderer->Initialize();

    auto civilizationView = GetApp().GetUniverse().view<cqspco::components::Civilization, cqspco::components::Player>();
...
void conquerspace::scene::UniverseScene::Update(float deltaTime) {
    ...
    if (!ImGui::GetIO().WantCaptureMouse && !game_halted) {
        ...
        entt::entity ent = GetApp().GetUniverse().view<conquerspace::client::systems::MouseOverEntity>().front();
