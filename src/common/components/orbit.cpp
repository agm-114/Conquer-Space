...
#include "orbit.h"
    ...
    glm::dvec3 ConvertToOrbitalVector(const double LAN, const double i, const double w, const double v,
                                      const glm::dvec3& vec) {
        return glm::dquat {glm::dvec3(0, 0, LAN)} * glm::dquat {glm::dvec3(i, 0, 0)} *
               glm::dquat {glm::dvec3(0, 0, w)} * glm::dquat {glm::dvec3(0, 0, v)} * vec;
    }
 ...
 */
    ...
    glm::dvec3 OrbitToVec3(const double& a, const double& e, const radian& i, const radian& LAN, const radian& w,
        ...
        return semi_param * ConvertToOrbitalVector(LAN, i, w, 0,
                                                   glm::dvec3(cos(v) / (1 + e * cos(v)), sin(v) / (1 + e * cos(v)), 0));
    }
 ...
 */
    ...
    double Orbit::GetOrbitingRadius(double v) const {
    ...
    double Orbit::TimeToTrueAnomaly(double v2) const {
        ...
        if (eccentricity > 1) {
            ...
            double E0 =
                std::atan2(std::sqrt(1.0 - eccentricity * eccentricity) * std::sin(v), eccentricity + std::cos(v));
            double M0 = E0 - std::sin(E0) * eccentricity;

            double E =
                std::atan2(std::sqrt(1.0 - eccentricity * eccentricity) * std::sin(v2), eccentricity + std::cos(v2));
